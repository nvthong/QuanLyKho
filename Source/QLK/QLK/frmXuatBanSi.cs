...
namespace QLK
{
    public partial class frmXuatBanSi : Form
    {
        ...
        public static frmXuatBanSi _frmNhapKho;
        DataTable dtHH;
        DataTable dtReport;
        ...
        public void fillDataHHbySHDNB(string pSoHDNB)
        {
            try
            {
                ...
                objList = ClassController.layDSHoaDonXuatSiTheoSHDNB(pSoHDNB);
                if (objList.Count > 0)
                {
        ...
        public void updateTienThoiLai()
        {
            try
            {
                ...
                vTongThanhToan = txtTongThanhToan.Text.Trim() != "" ? (int) double.Parse(txtTongThanhToan.Text.Trim()) : 0;
                ...
                vTienThoiLai = vTienKhachDua - (vTongThanhToan - vTienGiamKhac);
        ...
        public void updateDSHoaDon()
        {
            try
            {
                ...
                gridCtrlHoaDon.DataSource = ClassController.layDSHoaDonXuatSiTheoKhoangThoiGian(vPeriod);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        {
            gridCtrlHoaDon.DataSource = ClassController.layDSHoaDonXuatSiDSLeft();
        }
            ...
            try
            {
                ...
                else if (btnThemHH.Text == "Lưu")
                {
                    if (StatusButtonHH == "Them")
                    {
                        ...
                        if ((txtSoLuong.Text.Trim() == "" ? 0 : double.Parse(txtSoLuong.Text.Trim())) < 0)
                        {
                            chkTraHang.Checked = true;
                        }

                        if ((txtSoLuong.Text.Trim() == "" ? 0 : double.Parse(txtSoLuong.Text.Trim())) >= 0 || !chkTraHang.Checked)
                        {
                            ...
                            double vSoLuongXuat = 0;
                            for (int i = 0; i < dtHH.Rows.Count; i++)
                            {
                                if (dtHH.Rows[i]["HH_MAHANG"].ToString() == txtMaHang.Text.Trim())
                                {
                                    vSoLuongXuat += double.Parse(dtHH.Rows[i]["HDNX_SOLUONG"].ToString());
                                }
                            }

                            double vTonKho = 0;
                            vTonKho = ClassController.layTonKhoHangHoaKhiSua(txtMaHang.Text.Trim(), txtHoaDon.Text.Trim(), txtXuatKhoMa.Text.Trim());
                            if (vTonKho < (double.Parse(txtSoLuong.Text.Trim()) + vSoLuongXuat))
                            {
                                MessageBox.Show("Tồn kho không đủ xuất");
                                txtMaHang.Focus();
                                return;
                        ...
                        else
                        {
                            ...
                            listHH = ClassController.layDSHangHoaNhapKhoByMaHang(txtXuatKhoMa.Text.Trim(), objHDNX.HH_MAHANG);
                            ...
                            else if (listHH.Count > 1)
                            {
                                ...
                                foreach (var item in listHH)
                                {
                                    ...
                                    if (vSoLuongTon >= vSoLuongXuat)
                                    {
                                        ...
                                        dtHH.Rows.Add(
                                            chkTraHang.Checked ? 1 : 0, objHDNX.HH_MAHANG, objHH.HH_TENHANG, objDVT.DVT_TENDONVI,
                                            vSoLuongXuat,
                                            vChiecKhau,
                                            vTongChiecKhau,
                                            vGiaBan,
                    ...
                    else if (StatusButtonHH == "Sua")
                    {
                        ...
                        if (objHDNX.HDNX_SOLUONG >= 0 || !chkTraHang.Checked)
                        {
                            double vSoLuongXuat = 0;
                            for (int i = 0; i < dtHH.Rows.Count; i++)
                            {
        ...
        public void ThemHD()
        {
            try
            {
                if (btnThemHD.Text == "Thêm")
                {
                    ...
                    setEmptyFieldHH();
                    setEmptyFieldTT();
                    txtHoaDon.Text = ClassController.getSoHDNB("XS");
                    StatusButtonHD = "Them";
                    btnThemHD.Text = "Lưu";
                ...
                else if (btnThemHD.Text == "Lưu")
                {
                    ...
                    if (StatusButtonHD == "Them")
                    {
                        ...
                        obj.KH_MAKHO = txtXuatKhoMa.Text.Trim();
                        ...
                        for (int i = 0; i < dtHH.Rows.Count; i++)
                        {
                            ...
                            ClassController.themHoaDonXuatSi(obj);
                        }
                        btnThemHD.Text = "Thêm";
                        btnSuaHD.Text = "Sửa";
                        StatusButtonHD = "";
                    ...
                    else if (StatusButtonHD == "Sua")
                    {
                        ...
                        try
                        {
                            ...
                            objHDNX_OLD = ClassController.layThongTinHoaDonXuatSi(HDNX_SOHDNB);

                            using (SqlConnection connect = ClassController.ConnectDatabase())
                            {
                                connect.Open();
                                SqlCommand sqlCmd = new SqlCommand("UpdateHdXuatSi", connect);
                            ...
                            objHDNX_NEW.KH_MAKHO = txtXuatKhoMa.Text.Trim();
                            ...
                            for (int i = 0; i < dtHH.Rows.Count; i++)
                            {
                                ...
                                ClassController.capNhatHoaDonXuatSi(objHDNX_NEW);
                            }
                            btnThemHD.Text = "Thêm";
                            btnSuaHD.Text = "Sửa";
                            StatusButtonHD = "";
                ...
                if (HDNX_SOHDNB != "")
                {
                    ...
                    if (dialogResult == DialogResult.OK)
                    {
                        try
                        {
                            ...
                            {
                                connect.Open();
                                SqlCommand sqlCmd = new SqlCommand("DeleteHdXuatsi", connect);
                                sqlCmd.CommandTimeout = 1000;
                                sqlCmd.Parameters.AddWithValue("@HDNX_SOHDNB", HDNX_SOHDNB);
                                sqlCmd.CommandType = CommandType.StoredProcedure;
                                sqlCmd.ExecuteNonQuery();
        ...
        private void btnInHD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<HD_NHAPXUAT> objList = new List<HD_NHAPXUAT>();
                    objList = ClassController.layDSHoaDonXuatSiTheoSHDNB(txtHoaDon.Text);
                    if (objList.Count > 0)
                    {
                        decimal vTongBan = 0;
                        decimal vThanhTien = 0;
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (StatusButtonHD != "" && StatusButtonHH != "")
                {
                    if (txtXuatKhoMa.Text != "" || txtXuatKhoTen.Text != "")
                    {
                        ...
                        {
                            frmShowHangHoa frm = new frmShowHangHoa(txtXuatKhoMa.Text.Trim());
                            frm.ShowDialog(this);
                            if (frm.pHhMa != null)
                            {
                                txtDonViTinh.Text = frm.pHhDVT;
                                txtGiaBan.Text = ((int)double.Parse(frm.pHhGiaBanSi)).ToString();
                        ...
                        }else
                        {
                            ...
                            if (objHH != null && objHH.HH_MAHANG != "")
                            {
                                ...
                                txtGiaBan.Text = ((int)objHH.HH_GIABANSI).ToString();
                                txtTonKho.Text = ClassController.layTonKhoHangHoa(objHH.HH_MAHANG, txtXuatKhoMa.Text.Trim()).ToString();
                            }
                            else
                            {
                                frmShowHangHoa frm = new frmShowHangHoa(txtXuatKhoMa.Text.Trim());
