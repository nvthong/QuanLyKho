...
namespace QLK
{
    public partial class frmXuatKhac : Form
    {
        ...
        public static frmXuatKhac _frmNhapKho;
        DataTable dtHH;
        DataTable dtReport;
        ...
        public void fillDataHHbySHDNB(string pSoHDNB)
        {
            try
            {
                ...
                objList = ClassController.layDSHoaDonXuatKhacTheoSHDNB(pSoHDNB);
                if (objList.Count > 0)
                {
        ...
        public void updateDSHoaDon()
        {
            try
            {
                ...
                gridCtrlHoaDon.DataSource = ClassController.layDSHoaDonXuatKhacTheoKhoangThoiGian(vPeriod);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString());
        ...
        {
            gridCtrlHoaDon.DataSource = ClassController.layDSHoaDonXuatKhacDSLeft();
        }
        ...
        public void ThemHD()
        {
            if (btnThemHD.Text == "Thêm")
            {
                ...
                setEmptyFieldHH();
                setEmptyFieldTT();
                txtHoaDon.Text = ClassController.getSoHDNB("XC");
                StatusButtonHD = "Them";
                btnThemHD.Text = "Lưu";
            ...
            else if (btnThemHD.Text == "Lưu")
            {
                ...
                if (StatusButtonHD == "Them")
                {
                    ...
                    for (int i = 0; i < dtHH.Rows.Count; i++)
                    {
                        ...
                        ClassController.themHoaDonXuatKhac(obj);
                    }
                    btnThemHD.Text = "Thêm";
                    btnSuaHD.Text = "Sửa";
                    StatusButtonHD = "";
                ...
                else if (StatusButtonHD == "Sua")
                {
                    ...
                    try
                    {
                        ...
                        objHDNX_OLD = ClassController.layThongTinHoaDonXuatKhac(HDNX_SOHDNB);

                        using (SqlConnection connect = ClassController.ConnectDatabase())
                        {
                            connect.Open();
                            SqlCommand sqlCmd = new SqlCommand("UpdateHdXuatKhac", connect);
                        ...
                        for (int i = 0; i < dtHH.Rows.Count; i++)
                        {
                            ...
                            ClassController.capNhatHoaDonXuatKhac(objHDNX_NEW);
                        }
                        btnThemHD.Text = "Thêm";
                        btnSuaHD.Text = "Sửa";
                        StatusButtonHD = "";
            ...
            if (HDNX_SOHDNB != "")
            {
                ...
                if (dialogResult == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            connect.Open();
                            SqlCommand sqlCmd = new SqlCommand("DeleteHdXuatkhac", connect);
                            sqlCmd.CommandTimeout = 1000;
                            sqlCmd.Parameters.AddWithValue("@HDNX_SOHDNB", HDNX_SOHDNB);
                            sqlCmd.CommandType = CommandType.StoredProcedure;
                            sqlCmd.ExecuteNonQuery();
        ...
        private void btnInHD_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<HD_NHAPXUAT> objList = new List<HD_NHAPXUAT>();
                    objList = ClassController.layDSHoaDonXuatKhacTheoSHDNB(txtHoaDon.Text);
                    if (objList.Count > 0)
                    {
